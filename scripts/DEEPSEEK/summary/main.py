...
class RateLimiter:
    """Rate limiter for DeepSeek API"""
    def __init__(self):
        self.last_request_time = 0
        self.min_delay = 1.0  # Lower delay for DeepSeek
...
def estimate_tokens(text: str) -> int:
    """Token estimation for DeepSeek (approximately 4 characters = 1 token)"""
    return max(1, int(len(text) / 4))

def call_deepseek_api_with_retry(user_message: str, max_retries: int = 3) -> tuple:
    """DeepSeek API call with cost tracking"""
...
def main():
    ...
    with TerminalLogger(terminal_log_file) as logger:
        ...
        print(" DEEPSEEK-V3.1 SUMMARY-AIDED CONTEXT RETENTION TESTER")
        ...
        # Save final results
        results_file = os.path.join(OUTPUT_DIR, f"deepseek_summary_results_{timestamp}.json")
