...
def call_deepseek_api_with_retry(user_message: str, max_retries: int = 3) -> tuple:
    
    if not DEEPSEEK_API_KEY or DEEPSEEK_API_KEY == "key":
        return "ERROR: DeepSeek API key not set", 0, 0
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"     DeepSeek-V3.1 API call (Attempt {attempt + 1})")
...
def process_file(file_path: str, file_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        max_affordable_requests = min(len(prompt_items), 
                                    int(cost_tracker.get_remaining_budget() * 0.9 / 0.001))  
        ...
        for i, prompt_data in enumerate(prompt_items, 1):
            ...
            if i % 10 == 0 and not cost_tracker.can_afford_request():
            ...
            response, input_tokens, output_tokens = call_deepseek_api_with_retry(full_prompt)
...
def main():
    print(" DEEPSEEK-V3.1 BASELINE CONTEXT RETENTION TESTER")
    ...
    print(f" DeepSeek is very affordable - excellent value for testing!")
    
    if not DEEPSEEK_API_KEY or DEEPSEEK_API_KEY == "key":
        print("\n DeepSeek API key not found!")
        print("   Please set your API key in the DEEPSEEK_API_KEY variable")
        return
    ...
    for file_type, file_path in PROMPT_FILES.items():
        ...
        try:
            ...
            if result:
                ...
                if cost_tracker.get_remaining_budget() < 0.5:  
    ...
    output_file = os.path.join(RESULTS_DIR, f"deepseek_baseline_results_{timestamp}.json")
    final_output = {
        "experiment_type": "baseline_needle_in_haystack",
        "model": DEEPSEEK_CONFIG["model"],
        "timestamp": time.time(),
        "experiment_config": {
            "max_context_tokens": DEEPSEEK_CONFIG["max_context_tokens"],
            "temperature": DEEPSEEK_CONFIG["temperature"],
            "top_p": DEEPSEEK_CONFIG["top_p"],
            "max_tokens": DEEPSEEK_CONFIG["max_tokens"]
        },
