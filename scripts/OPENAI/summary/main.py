...
class RateLimiter:
    ...
    def __init__(self):
        ...
        self.min_delay = 1.0  # Standard delay for OpenAI
...
def estimate_tokens(text: str) -> int:
    """Token estimation for OpenAI (approximately 4 characters = 1 token)"""
    return max(1, int(len(text) / 4))

def call_openai_api_with_retry(user_message: str, max_retries: int = 3) -> tuple:
    """OpenAI API call with cost tracking"""
...
def main():
    ...
    with TerminalLogger(terminal_log_file) as logger:
        ...
        print(" OPENAI GPT-4 TURBO SUMMARY-AIDED CONTEXT RETENTION TESTER")
        ...
        # Save final results
        results_file = os.path.join(OUTPUT_DIR, f"openai_summary_results_{timestamp}.json")
