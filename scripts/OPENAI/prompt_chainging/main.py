...
# === OPENAI GPT-4 TURBO Model Configuration ===
OPENAI_CONFIG = {
...
RATE_LIMIT_CONFIG = {
    "requests_per_minute": 5,  # Conservative rate limiting for GPT-4
    "max_retries": 2,
    "base_delay": 5,
    "max_delay": 30,
    "cost_per_million_input": 10.0,   # GPT-4 Turbo pricing
    "cost_per_million_output": 30.0,  # GPT-4 Turbo pricing (higher output cost)
    "max_budget": 5.0,  # Higher budget for GPT-4
    "safety_margin": 0.5
}

# === FILE PATHS ===
PROMPT_FILES = [
    "data\prompt_chainig_numeric_based.json",
    "data\prompt_chainig_text_based.json"
]

OUTPUT_DIR = "OPENAI\prompt_chainging"
...
def call_openai_api_with_retry(messages: List[Dict]) -> tuple[str, int, int]:
    """OpenAI API call with budget and rate limiting"""
    ...
    for attempt in range(RATE_LIMIT_CONFIG["max_retries"]):
        try:
            ...
            print(f"    OpenAI API call (Attempt {attempt + 1})")
    ...
    try:
        ...
        for chain_idx, chain in enumerate(chains, 1):
            ...
            for prompt_idx, prompt in enumerate(prompts, 1):
                ...
                response, input_tokens, output_tokens = call_openai_api_with_retry(messages)
...
def main():
    ...
    try:
        print("OPENAI GPT-4 TURBO PROMPT CHAINING EVALUATOR")
        ...
        if OPENAI_API_KEY == "key":
            print("\nPlease set your OPENAI_API_KEY in the script!")
            print("   You can get it from: https://platform.openai.com/")
            return
        ...
        print(f"\n{'='*60}")
        print(f"FINAL RESULTS - OPENAI GPT-4 TURBO PROMPT CHAINING")
        print(f"{'='*60}")
        ...
        remaining_budget = budget_tracker.max_budget - budget_tracker.current_cost
        print(f"Remaining budget: ${remaining_budget:.4f}")
        print("OpenAI GPT-4 Turbo prompt chaining evaluation completed!")
