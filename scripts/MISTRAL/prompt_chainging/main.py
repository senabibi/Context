...
MISTRAL_API_KEY = "key"  # Replace with your actual API key

# === MISTRAL MIXTRAL 8x7B Model Configuration ===
MISTRAL_CONFIG = {
...
RATE_LIMIT_CONFIG = {
    ...
    "base_delay": 5,
    "max_delay": 30,
    "cost_per_million_input": 0.7,   # Mistral pricing
    "cost_per_million_output": 0.7,  # Mistral pricing (same for input/output)
    "max_budget": 2.0,  # Budget for Mistral
    "safety_margin": 0.2
}

# === FILE PATHS ===
PROMPT_FILES = [
    r"C:\Users\bitir\OneDrive\Desktop\Retentions\data\prompt_chainig_numeric_based.json",
    r"C:\Users\bitir\OneDrive\Desktop\Retentions\data\prompt_chainig_text_based.json"
]

OUTPUT_DIR = r"C:\Users\bitir\OneDrive\Desktop\Retentions\scripts\MISTRAL\prompt_chainging"
...
class TerminalLogger:
    def __init__(self, output_dir: str):
        ...
        self.log_file = os.path.join(output_dir, f"terminal_results_{timestamp}.txt")
...
def call_mistral_api_with_retry(messages: List[Dict]) -> tuple[str, int, int]:
    """Mistral API call with budget and rate limiting"""
    ...
    try:
        ...
        for chain_idx, chain in enumerate(chains, 1):
            ...
            for prompt_idx, prompt in enumerate(prompts, 1):
                ...
                response, input_tokens, output_tokens = call_mistral_api_with_retry(messages)
...
def save_results(all_results: List[Dict], output_dir: str):
    ...
    if numeric_results:
        numeric_file = os.path.join(output_dir, f"numeric_based_results_{timestamp}.json")
    ...
    # Save text results
    if text_results:
        text_file = os.path.join(output_dir, f"text_based_results_{timestamp}.json")
...
def main():
    ...
    try:
        print("MISTRAL MIXTRAL 8x7B PROMPT CHAINING EVALUATOR")
        ...
        if MISTRAL_API_KEY == "key":
            print("\nPlease set your MISTRAL_API_KEY in the script!")
            print("   You can get it from: https://console.mistral.ai/")
            return
        ...
        print(f"\n{'='*60}")
        print(f"FINAL RESULTS - MISTRAL MIXTRAL 8x7B PROMPT CHAINING")
        print(f"{'='*60}")
        ...
        remaining_budget = budget_tracker.max_budget - budget_tracker.current_cost
        print(f"Remaining budget: ${remaining_budget:.4f}")
        print("Mistral Mixtral 8x7B prompt chaining evaluation completed!")
