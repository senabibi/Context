...
# === MISTRAL MIXTRAL 8X7B CONFIGURATION ===
...
class RateLimiter:
    ...
    def wait_if_needed(self):
        ...
        if elapsed < self.min_delay:
            ...
            print(f" Rate limiting: {wait_time:.1f}s bekle...")
...
def call_mistral_api_with_retry(user_message: str, max_retries: int = 3) -> tuple:
    ...
    if not MISTRAL_API_KEY or MISTRAL_API_KEY == "key":
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"     Mistral Mixtral 8x7B API call (Attempt {attempt + 1})")
...
def save_checkpoint(results: Dict, checkpoint_file: str = None):
    if checkpoint_file is None:
        checkpoint_file = os.path.join(RESULTS_DIR, "mistral_checkpoint.json")
...
def process_file(file_path: str, file_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        max_affordable_requests = min(len(prompt_items), 
                                    int(cost_tracker.get_remaining_budget() * 0.9 / 0.002))  # Ortalama 0.002$ per request for Mistral
        ...
        for i, prompt_data in enumerate(prompt_items, 1):
            ...
            if i % 5 == 0 and not cost_tracker.can_afford_request():
...
def main():
    print(" MISTRAL MIXTRAL 8X7B BASELINE CONTEXT RETENTION TESTER")
    ...
    print(f" Mistral Mixtral offers good value for testing!")
    ...
    for file_type, file_path in PROMPT_FILES.items():
        ...
        try:
            ...
            if result:
                ...
                if cost_tracker.get_remaining_budget() < 1.0:  
    ...
    output_file = os.path.join(RESULTS_DIR, f"mistral_baseline_results_{timestamp}.json")
    final_output = {
        "experiment_type": "baseline_needle_in_haystack",
        "model": MISTRAL_CONFIG["model"],
        "timestamp": time.time(),
        "experiment_config": {
            "max_context_tokens": MISTRAL_CONFIG["max_context_tokens"],
            "temperature": MISTRAL_CONFIG["temperature"],
            "top_p": MISTRAL_CONFIG["top_p"],
            "max_tokens": MISTRAL_CONFIG["max_tokens"]
        },
