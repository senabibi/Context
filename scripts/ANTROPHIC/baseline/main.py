...
# === ANTHROPIC CLAUDE 3 OPUS CONFIGURATION ===
...
# === COST TRACKING (Claude 3 Opus pricing) ===
PRICING = {
...
# === FILE PATHS ===
BASE_DIR = "scripts\ANTROPHIC\baseline"
DATA_DIR = "data"
...
# === PROMPT FILES ===
PROMPT_FILES = {
    "numeric": os.path.join(DATA_DIR, "data\baseline_numeric_base.json"),
    "text": os.path.join(DATA_DIR, "data\baseline_text_base.json")
}
...
def call_claude_api_with_retry(user_message: str, max_retries: int = 3) -> tuple:
    
    if not ANTHROPIC_API_KEY or ANTHROPIC_API_KEY == "key":
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Claude 3 Opus API call (Attempt {attempt + 1})")
...
def save_checkpoint(results: Dict, checkpoint_file: str = None):
    if checkpoint_file is None:
        checkpoint_file = os.path.join(RESULTS_DIR, "claude_opus_checkpoint.json")
...
def process_file(file_path: str, file_type: str) -> Dict[str, Any]:
    ...
    try:
        ...
        max_affordable_requests = min(len(prompt_items), 
                                    int(cost_tracker.get_remaining_budget() * 0.8 / 0.02))  
...
def main():
    print(" CLAUDE 3 OPUS BASELINE CONTEXT RETENTION TESTER")
    ...
    print(f" Claude 3 Opus is expensive - budget carefully!")
    
    if not ANTHROPIC_API_KEY or ANTHROPIC_API_KEY == "key":
        print("\n Anthropic API key not found!")
        print("   Please set your API key in the ANTHROPIC_API_KEY variable")
        return
    ...
    output_file = os.path.join(RESULTS_DIR, f"claude_opus_baseline_results_{timestamp}.json")
    final_output = {
        "experiment_type": "baseline_needle_in_haystack",
        "model": CLAUDE_CONFIG["model"],
        "timestamp": time.time(),
        "experiment_config": {
            "max_context_tokens": CLAUDE_CONFIG["max_context_tokens"],
            "temperature": CLAUDE_CONFIG["temperature"],
            "top_p": CLAUDE_CONFIG["top_p"],
            "max_tokens": CLAUDE_CONFIG["max_tokens"]
        },
