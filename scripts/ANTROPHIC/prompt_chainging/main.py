...
OUTPUT_DIR = "ANTROPHIC\prompt_chainging"
...
class BudgetTracker:
    ...
    def can_afford_request(self, estimated_input_tokens: int, estimated_output_tokens: int = 1024) -> bool:
...
def call_claude_api_with_retry(messages: List[Dict]) -> tuple[str, int, int]:
    """Claude API call with budget and rate limiting"""
    ...
    for attempt in range(RATE_LIMIT_CONFIG["max_retries"]):
        try:
            ...
            for i, msg in enumerate(claude_messages):
                ...
                elif msg["role"] == "assistant" and formatted_messages:
                    formatted_messages.append(msg)
            
            if formatted_messages and formatted_messages[-1]["role"] == "assistant":
                formatted_messages.append({"role": "user", "content": "Please continue or provide your final answer."})
            ...
            print(f"    Claude API call (Attempt {attempt + 1})")
            ...
            if response.status_code == 200:
                ...
                output_tokens = usage.get("output_tokens", budget_tracker.estimate_tokens(content))
    ...
    try:
        ...
        for chain_idx, chain in enumerate(chains, 1):
            ...
            for prompt_idx, prompt in enumerate(prompts, 1):
                ...
                response, input_tokens, output_tokens = call_claude_api_with_retry(messages)
...
def main():
    ...
    try:
        print("CLAUDE 3.5 SONNET PROMPT CHAINING EVALUATOR")
        ...
        if ANTHROPIC_API_KEY == "key":
            print("\nPlease set your ANTHROPIC_API_KEY in the script!")
        ...
        print(f"\n{'='*60}")
        print(f"FINAL RESULTS - CLAUDE 3.5 SONNET PROMPT CHAINING")
        print(f"{'='*60}")
        ...
        remaining_budget = budget_tracker.max_budget - budget_tracker.current_cost
        print(f"Remaining budget: ${remaining_budget:.4f}")
        print("Claude 3.5 Sonnet prompt chaining evaluation completed!")
